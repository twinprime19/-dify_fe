  ...
  const handleCredentialLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        console.log('Login successful, NextAuth should handle redirect automatically');
      } else {
          ...
          {Object.values(providers)
            ...
            .map((provider) => (
              <button
                ...
                className={`w-full flex items-center justify-center gap-3 py-2.5 border rounded-md hover:bg-gray-50 transition ${provider.id === 'google' ? 'text-gray-700' : 'text-white bg-blue-500 hover:bg-blue-600'
                  }`}
              >
                {provider.id === 'google' && (
